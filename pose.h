...
float Co_ref(Pose pose_refer, float angulo){

	Pose anterior;
	anterior.x = 0;
	anterior.y = 0;
	anterior.theta = 0;
	float hip_referencia;
	float cat_oposto;
	hip_referencia = calc_dist(pose_refer, anterior);
	cat_oposto = (sin(angulo)*hip_referencia);
	
	return cat_oposto;
}                                                                                                                              
float Co_real(float co_ref, float sonar, float hipotenusa){
	return asin((sonar - co_ref)/hipotenusa);
}
// angulo real do robô
